 ...
 */
public class AmexFormatterStrategy implements FormatStrategy {
    @Override
    public void formatCardNumber(Editable editable) {
        if (editable.length() == 5 || editable.length() == 12) {
            ...
            } else if (Character.isDigit(c) && TextUtils.split(editable.toString(), String.valueOf(CreditCardConstants.DEFAULT_SEPERATOR)).length <= 3) {
